...
```
Then you should be able just import and using classes from `src/Library`.

### Using form model

```ts
import { FormGroup, FormControl, FormArray } from 'src/Library';
...
```ts
import { FormBuilder, Control } from 'src/Library';
...
Since version 1.1.0, `src/Library` supports `input[type=file]`.
...
```ts
import { Validators, FormControl } from 'src/Library';
...
```ts
import { FormControl, FormGroup, FormArray } from 'src/Library';
...
Native `ValidatorFn` and `AsyncValidatorFn` are interfaces, in `src/Library` they are types.